...
//Additional notes
//•buses cannot outrun buses in front of them ◦If bus A is moving faster and reaches a bus B that is moving slower, then A must lower its speed to match that of B

//•buses with equal speeds does not form a group ◦Their form different groups
...
namespace _02.Buses
{
   class Buses
   {
      static void Main()
      {
         int count = int.Parse(Console.ReadLine()); //count of busses
         int groups = 0;
         ...
         for (int i = 0; i < count; i++)
         {
            ...
            {
               groups++; // the groups are increasing
               groupSpeed = busSpeed; // the group speed is decreasing to the speed of the first bus or if the bus is slower, than it is driving with its own speed and is forming a different group
            }
         }
