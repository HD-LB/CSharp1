...
namespace _11.SaddyKopper
{
   class SaddyKopper
   {
      static void Main()
      {
         ...
         {
            BigInteger product = 1; //product always = 1 (only sum = 0;)


            //logic

            while (strNum.Length > 1 ) //otherwise the product will be 0
            {
               strNum = strNum.Substring(0, strNum.Length - 1); // 2.Step, takes the first index to the one before the last one. 
               int sum = 0;

               for (int i = 0; i < strNum.Length; i += 2) // i = 0 and i += 2, as per instructiona only the even numbers
               {
                  sum += (strNum[i] - '0'); // 3.Step, that is a char, so we have to take '0' out to get an int
               }
               product *= sum; //!= 0 ? sum : 1;
            }
            transCount++;
            strNum = product.ToString(); //6.Step
         }
